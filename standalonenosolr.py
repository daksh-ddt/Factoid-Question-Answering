...
import alchemynosolr


class StandAloneTestNoSolr:
    def __init__(self):
        pass
    ...
    def runStandAloneTest(self, question):
        ...
        keywordsList = querybuilder.buildKeywordsList(
            question[0], '/home/gavin/dev/Factoid-Question-Answering')
        print "Keywords: %s\n" % keywordsList

        query = querybuilder.buildQueryString(keywordsList)
        print "Query: %s\n" % query

        classifier = answertype.Classifier('/home/gavin/dev/Factoid-Question-Answering')
        answerType = classifier.classifyAnswerType(question)

        data = binginterface.search("'" + query + "'")
        document_descriptions = documentcreator.getDescriptions(data, 5)
...
#            print "Result: %s" % result['text']
        ...
        rankedAnswerCandidates = alchemynosolr.parseEntities(filteredDocuments, answerType)
...
if __name__ == "__main__":
    ...
    StandAloneTestNoSolr().runStandAloneTest(argv)
